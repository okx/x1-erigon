...
install_linux() {
	...
	$sh_c "cd jemalloc && ./autogen.sh" #--with-jemalloc-prefix='je_'
	$sh_c "cd jemalloc && ./configure --prefix=/usr --libdir=/usr/lib"#--with-private_namespace
...
do_install() {
	...
	case "$lsb_dist" in
		...
		centos)
			...
			;;
	  alpine)
	    pre_reqs="git make autoconf automake libtool gcc g++ linux-headers"
      $sh_c "apk add --no-cache $pre_reqs"
      install_linux
      exit 0
	    ;;
		*)
